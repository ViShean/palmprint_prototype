...
def run_darknet_detection(
    ...
    if not os.path.isfile(darknet_exe):
        print("Error: 'darknet.exe' not found in directory: {}".format(darknet_dir))
        return []
    ...
    while tries < max_tries:
        tries += 1
        print("\nAttempt {}: Running detection with threshold {}".format(tries,threshold))
        command = [
            darknet_exe,
            "detector",
            "test",
        ...
        try:
            ...
            raw_output = result.stdout.decode("utf-8")  # Convert bytes to string
            detections = parse_darknet_output(raw_output)
            detected_points = len(detections)
            print("Detected points before NMS: {}".format(detected_points))

            if detected_points == 0:
                print("No detections found for {} with threshold {}.".format(input_image,threshold))
                threshold -= 0.1  # Decrease threshold to allow more detections
                if threshold < min_threshold:
                    print("Threshold below minimum limit ({}). Stopping detection.".format(min_threshold))
                    break
                continue
            ...
            if len(indices) == 0:
                print("All detections suppressed after NMS for {}. Adjusting threshold.".format(input_image))
            ...
            print("NMS filtered boxes count: {}".format(len(nms_boxes)))
            ...
            if len(nms_boxes) == 4:
                print("Successfully detected 4 boxes on attempt {} with threshold {}".format(tries,threshold))
                ...
                if threshold > min_threshold:
                    print("Detected points ({}) less than 4. Decreasing threshold.".format(len(nms_boxes)))
                    threshold -= 0.05  # Decrease threshold to allow more detections
                else:
                    print("Threshold reached minimum limit ({}). Cannot detect 4 boxes.".format(min_threshold))
                    break
            else:
                # More than 4 boxes but already handled by selecting top 4
                print("Tried: {}, Threshold: {}".format(tries,threshold))
                final_detections = []
                for det in nms_detections:
                    detection = {
                        'class_name': det['class_name'],  # Use actual class name
            ...
            print("An error occurred while running the detection command for {}:".format(input_image))
            print(e.stderr)
            return []

    print("Exceeded maximum tries for {}. Returning available detections.".format(input_image))
    return detections
